...
sequenceDiagram
    ...
    alt Попадание в кэш (Cache Hit)
        ...
        alt Успешная генерация
            LLM-->>Router: Сгенерированный ответ (сырой текст)
...
* **Реализация в прокси:**
  ...
  * Модуль NLI проверяет сгенерированный ответ суфлера по тексту нормативного документа перед показом оператору.
