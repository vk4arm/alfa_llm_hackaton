...
function initAlfaApp() {
  ...
  const nodeSpecs = {
    ...
    'kafka': {
      title: 'KUMA SIEM & WORM Audit',
      sub: 'Централизованная SIEM-система банка (CEF формат)',
      icon: '🛡️',
      details: `<p>Все события маскирования ПДн (152-ФЗ), отражения Jailbreak, галлюцинаций NLI и срабатывания Circuit Breaker транслируются в <strong>KUMA SIEM</strong> (совместимо со Splunk / QRadar) в формате CEF (Common Event Format) через асинхронный WORM-логгер.</p>`
    },
    'vectordb': {
